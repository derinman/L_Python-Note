...
class Simulator:
    ...
    def run(self, end_time):
        ...
        while sim_time < end_time:
            ...
            current_event = self.events.get() #取得時間最小的的event設為current_event
            sim_time, proc_id, previous_action = current_event
            print('taxi:', proc_id, proc_id * '   ', current_event)  #印出
            active_proc = self.procs[proc_id] 
            next_time = sim_time + compute_duration(previous_action)  #算出下個活動時間
            try:
                next_event = active_proc.send(next_time)
            except StopIteration:
                del self.procs[proc_id]  #該協程結束，刪除self.procs的該協程
            else:
